...
+ Логика первого порядка с кванторами (FOL)
...
Синтаксис языка предикатов определяется на @f-design-1 и исходит напрямую из логики первого порядка без кванторов с неинтерпретируемыми функциями и линейной арифметикой целых чисел. Единственным исключением являются переменные Хорна @rondon2008liquid, которые требуются для вывода предикатов в уточняющих типов. Они будет рассмотрены позже.
...
которая принимает тип-уточнение, функцию которая модифицирует предикат и в итоге конструируется новый тип, с новым предикатом. В дальнейшем этот контракт будет расширен и его применение будет показано  для решения широкого ряда задач от вывода предикатов, до реализации функций усиливающих предикат.

=== Сигнатура типа
Сорта (sorts) в контексте SMT — это аналоги типов данных в языках программирования @Barrett2010SMTLIB. Они определяют множество значений, которые могут принимать переменные, аргументы функций или возвращаемые значения. Каждый сорт задаёт область допустимых значений и правила взаимодействия с операциями теории.
...
В конечном итоге для проверки уточняющих типов нужно в сгенерированных условиях корректности сопоставить для каждой переменной Хорна свой предикат без переменных и так чтобы получившиеся ограничения были верны, если же такого вывода нет, то результат будет Unsat. Алгоритм представленный ниже основывается на работе Liquid Types @rondon2008liquid.
