...
Данная работа сильно полагается на два инструмента написанных на языке Haskell и представленных ниже.
...
Концепция free foil @kudasov2024free использует идею предложенную в foil @maclaurin2022foil для отслеживания областей видимости. Каждое выражение параметризуется фантомным типом `n :: S` который определяет область в которой это выражение используется. Используя возможности системы типов Haskell, в момент компиляции проверяется, что работа с выражениями идет строго с учетом их области видимости.
...
+ Подстановка: имея показанную на @f-realization-1 структуру синтаксического дерева можно предоставить обобщенную подстановку не зависящую от модельного языка. Подстановка реализована на основе класса `Sinkable` гарантирующего безопасное переименование переменных c  учетом областей видимости.
...
+ Универсальность: поддерживает произвольные языки с помощью пользовательских сигнатур. Позволяя реализовывать универсальные алгоритмы (например, подстановку), которые работают на разных языках без повторной реализации
...
liquid-fixpoint @liquid-fixpoint это фреймворк для решения ограничений в формате CHC (Constrained Horn clause).
