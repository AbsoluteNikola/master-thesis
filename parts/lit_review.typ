...
=== LiquidHaskell
LiquidHaskell @vazou2014refinement - это система верификации и проверки типов для языка Haskell, которая расширяет систему типов языка добавлением уточняющих типов в виде аннотаций в программный код. Система предикатов поддерживает логику первого порядка без кванторов с неинтерпретируемыми функциями и линейной арифметикой. Для проверки соблюдения свойств программы используется SMT-Решатель. Применяется как плагин к компилятору GHC, что позволяет легко интегрировать систему в существующие проекты. Пример кода показан на @lit-1.
#figure(
  ```haskell
...
Особенности:
  - возможность использовать систему в ограниченных участках кода без необходимости вносить изменения в весь проект;
  ...
  - мощность языка предикатов специально ограниченна в угоду автоматизации по сравнению с другими инструментами.

=== F\*
F\* @swamy2016dependent - это функциональный язык программирования с зависимыми типами, разработанный Microsoft Research для формальной верификации программ. Основной его особенностью является генерация верифицированного кода в код на языках C, OCaml, F\# и WebAssembly, сохраняя доказанные свойства.

F\* использует зависимые типы, дополняя их уточнениями, которые могут содержать логику высшего порядка. Такой выбор системы типов дает мощные инструменты для верификации программного обеспечения, но требует от программиста больших усилий для доказательства корректности, а также направления проверки типов в нужное русло. Поэтому язык используется для верификации критически важных систем, с последующей генерацией кода в один из предоставленных языков программирования.

В связи с этим применять язык F\* как язык общего назначения проблематично.
...
=== Dafny
Dafny @leino2010dafny - это язык программирования, ориентированный на разработку верифицированных программ.

Dafny сочетает в себе возможности императивного, функционального и объектно-ориентированного программирования с богатой системой статической верификации.
...
=== Why3
Why3 @araiza2014formal - это система верификации программ, разработанная специально для проверки алгоритмического уровня, а не программного. Why3 предлагает свой язык WhyML, который сочетает функциональный стиль программирования с императивными конструкциями.

В Why3 спецификации записываются в виде пред- и постусловий функций, аналогично Dafny. Однако Why3 также поддерживает механизм уточнения через клонирование модулей, что позволяет реализовывать абстрактные спецификации и постепенно уточнять их. Why3 генерирует условия верификации, которые затем проверяются с помощью различных SMT-решателей. Система поддерживает интеграцию с различными решателями, включая Z3, Alt-Ergo, CVC4.
